...
class SynthesizerIncremental(Optimizer):
    ...
    def __init__(self, target_dynamic_path, target_static_path, config):
        ...
        with self.graph.as_default():
            with tf.device('/gpu:' + str(self.user_config['gpu'])):
                ...
                # initialize first frame to be the static texture
                first_frame = tf.expand_dims(vgg_process(
                    self.target_static_texture), axis=0)
                first_frame_batch = tf.tile(first_frame,
                                            [self.user_config['batch_size'],
                                             1, 1, 1, 1])

                self.output = tf.concat([first_frame_batch,
                                         tf.Variable(initial_noise)], 1,
                                        name='output')
